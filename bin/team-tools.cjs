...
/**
 * Agent Teams Tools — CLI utility for Agent Teams workflow operations
 *
 * Replaces repetitive inline bash patterns across Agent Teams command/workflow/agent files.
 * Centralizes: config parsing, phase lookup, git commits, summary verification.
 *
 * Usage: node team-tools.cjs <command> [args] [--raw]
 *
 * Atomic Commands:
...
async function main() {
  ...
  if (!command) {
    error('Usage: team-tools <command> [args] [--raw] [--cwd <path>]\nCommands: state, find-phase, commit, verify-summary, verify, frontmatter, template, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section, init');
  }
  ...
  switch (command) {
    ...
    case 'commit': {
      ...
      // then join them (handles both quoted and unquoted invocations)
